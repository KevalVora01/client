...
const schema = Yup.object({
  ...
  residentId: Yup.number().nullable(),
...
const BookingFormModal = ({
  ...
  const formik = useFormik({
    initialValues: {
      ...
      startTime: initialStart ?? '08:00',
      endTime: initialEnd ?? '09:00',
      purpose: '',
      residentId: '',
      apartmentId: '',
    },
    ...
    onSubmit: async (values) => {
      ...
      if (isAdmin) {
        ...
        if (values.apartmentId !== '') payload.apartmentId = Number(values.apartmentId);
      }
      ...
      <div className="modal-dialog modal-lg modal-dialog-centered modal-dialog-scrollable" onClick={(e) => e.stopPropagation()}>
        <div className="modal-content border-0 rounded-3 shadow-lg bg-white">
          <div className="modal-header border-bottom border-light-subtle px-3 px-sm-4 pt-4 pb-3 align-items-start position-relative">
            <div>
              <h5 className="modal-title fw-bold fs-6" style={{ color: '#1a1f36' }}>Book Amenity</h5>
              <p className="text-muted mb-0" style={{ fontSize: '0.8rem' }}>Pick a date and time slot.</p>
            </div>
            <button
              type="button"
            ...
            <form onSubmit={formik.handleSubmit}>
              <div className="row g-3">
                <div className="col-12 col-md-4">
                  ...
                  <input type="date" name="bookingDate" className={fieldClass('bookingDate')} value={formik.values.bookingDate} onChange={formik.handleChange} onBlur={formik.handleBlur} style={{ fontSize: '0.875rem', height: '40px' }} />
                  {formik.touched.bookingDate && formik.errors.bookingDate && <div className="invalid-feedback">{formik.errors.bookingDate}</div>}
                </div>
                <div className="col-12 col-md-4">
                ...
                <div className="col-12">
                  ...
                  <textarea name="purpose" rows={2} className={fieldClass('purpose')} value={formik.values.purpose} onChange={formik.handleChange} onBlur={formik.handleBlur} style={{ fontSize: '0.875rem', resize: 'vertical' }} />
                </div>

                {isAdmin && (
                  <>
                    <div className="col-12 col-md-6">
                      <label className="form-label fw-medium text-secondary small mb-1">Resident ID (on behalf)</label>
                      <input type="number" name="residentId" className={fieldClass('residentId')} value={formik.values.residentId} onChange={formik.handleChange} onBlur={formik.handleBlur} style={{ fontSize: '0.875rem', height: '40px' }} />
                    </div>
                    <div className="col-12 col-md-6">
                      <label className="form-label fw-medium text-secondary small mb-1">Apartment ID</label>
                      <input type="number" name="apartmentId" className={fieldClass('apartmentId')} value={formik.values.apartmentId} onChange={formik.handleChange} onBlur={formik.handleBlur} style={{ fontSize: '0.875rem', height: '40px' }} />
                    </div>
                  </>
                )}
                ...
                <div className="col-12">
                  <div className="d-grid d-sm-flex gap-2 justify-content-sm-end">
                    ...
                    <button type="submit" className="btn btn-dark fw-medium px-3 d-inline-flex align-items-center justify-content-center" disabled={loading} style={{ height: '38px', fontSize: '0.875rem', borderRadius: '8px', opacity: loading ? 0.55 : 1 }}>
                      {loading ? <span className="spinner-border spinner-border-sm" /> : <><i className="bi bi-check-lg me-1" /> Submit Request</>}
                    </button>
                  </div>
                </div>
              </div>
